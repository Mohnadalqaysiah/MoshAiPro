...
def _recommendations(levers: dict, labels: dict, total_r: float,
    ...
    # (2026-09-16) تداخل الفرضيات. رافعات مختلفة قد تصف نفس الصفقات:
    # تشغيل 7 أيام أعطى ثلاث فرضيات من أصل 107 قرار، والعرض يوحي بأن
    # آثارها تُجمع. لا تُجمع إن كانت نفس الصفقات موصوفة مرتين.
    #
    # (2026-09-16 #2) الصيغة الأولى كانت نسبة خام (التقاطع ÷ الأصغر)
    # بعتبة 50% — وهي خاطئة بنيوياً: شريحة تشكّل 53% من القرارات
    # (BUY مثلاً) ستتقاطع مع أي شريحة أخرى بـ~53% بمحض الصدفة، فيُطلق
    # الإنذار على استقلال تام. وقد حصل فعلاً بتشغيل 30 يوماً: وُسمت
    # "R/R < 1.5" بأنها نفس صفقات BUY بنسبة 50% بينما معدّل BUY الأساسي
    # 53.5% — أي تقاطعها أقل من الصدفة، ومع ذلك حُذّر منها. والإنذار
    # الكاذب أسوأ من غيابه: يدفع لتجاهل التحذيرات الحقيقية.
    # الصحيح رفع الاستقلال (lift): التقاطع المرصود ÷ المتوقَّع لو كانت
    # الشريحتان مستقلتين. 1.0 = صدفة محضة، و>1 = ترابط فعلي.
    N = max(total_n, 1)
    for i, a in enumerate(recs):
        ...
        for j, b in enumerate(recs):
            ...
            if not inter:
                continue
            na, nb = len(a["_ids"]), len(b["_ids"])
            observed = inter / na
            lift     = (inter * N) / (na * nb) if na and nb else 0.0
            if lift >= OVERLAP_LIFT and observed >= 0.4:
                overlaps.append("%s: %s (%d%% منها — %.1f× الصدفة)"
                                % (b["lever"], b["value"], int(observed * 100), lift))
        a["overlaps"] = overlaps
