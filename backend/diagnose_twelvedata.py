...
def main():
    ...
    print("=" * 84)
    print("  ⚠️ ملاحظة: هذه عملية مستقلة عن السيرفر — SmartData.__init__ يبدأ")
    print("     دائماً بـ_td_enabled=False، فالقيمة قبل الاستعادة لا تعكس حالة")
    print("     السيرفر إطلاقاً (سوء فهم حصل فعلاً بتاريخ 15/09).")
    print(f"  قبل الاستعادة: _td_enabled  = {smart_data._td_enabled}")
    restored = smart_data.restore_twelvedata_from_settings()
    print(f"  بعد الاستعادة: _td_enabled  = {smart_data._td_enabled}  (نجحت={restored})")
